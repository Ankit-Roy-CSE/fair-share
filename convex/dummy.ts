...
import { GroupData, GroupExpenseData, ExpenseData } from "./types";
import { MutationCtx } from "./_generated/server";
import { Doc , Id } from "./_generated/dataModel";

/**
 * Seed database with dummy data using your existing users
 * Run with: npx convex run dummy:seedDb
 */
export const seedDb = mutation({
  args: {},
  handler: async (ctx) => {
    ...
    const IndividualExpenses = await createIndividualExpenses(ctx, users);
...
async function createGroups(
    ...
    return await Promise.all(
        groupIds.map(async (id) => {
            ...
            return { ...group, _id: id } as  Doc<"groups">;
        })
    );
}

// Helper to create one-on-one expenses
async function createIndividualExpenses(
    ctx: MutationCtx,
    users: Doc<"users">[]
    ...
    // Define the IndividualExpenseData type if not already imported
    

    const expenseDatas: ExpenseData[] = [
        {
    ...
    return await Promise.all(
        expenseIds.map(async (id) => {
            ...
            return { ...expense, _id: id } as Doc<"expenses">;
        })
    );
}
...
async function createGroupExpenses(
    ...
    // Office Expenses
    const officeExpenses: GroupExpenseData[] = [
        {
    ...
    const projectExpenses: ExpenseData[] = [
        {
    ...
    const allGroupExpenses: ExpenseData[] = [
        ...weekendTripExpenses,
        ...officeExpenses,
        ...projectExpenses,
    ];
...
async function createSettlements(
  ...
  users : Doc<"users">[],
  groups : Doc<"groups">[],
  individualExpenses : Doc<"expenses">[],
  groupExpenses : Doc<"expenses">[]
) {
  const now = Date.now();
  ...
  const cabExpense = individualExpenses.find(
