...
import { EnhancedBalance, EnhancedGroupMember } from "./types";
...
export const getGroupExpenses = query({
  ...
  handler: async (ctx, { groupId }) => {
    ...
    /* ----------  member map ---------- */
    const memberDetails : EnhancedGroupMember[] = (
      await Promise.all(
    ...
    ).filter((m): m is EnhancedGroupMember => m !== undefined);

    if (memberDetails.some((m) => m?.id === null)) {
      throw new Error("Some members no longer exist");
    }
    const ids = memberDetails.map((m) => m.id?.toString() || "");

    /* ----------  ledgers ---------- */
    // total net balance (old behaviour)
    const totals = Object.fromEntries(ids.map((id) => [id, 0]));
    // pair‑wise ledger  debtor -> creditor -> amount
    const ledger: Record<string, Record<string, number>> = {}; // The ledger is basically adjacency matrix of the debt graph
    // Initialize the ledger with 0 for each pair of members
    ids.forEach((a) => {
      ledger[a] = {};
      ids.forEach((b) => {
        if (a !== b) ledger[a][b] = 0;
    ...
    ids.forEach((a) => {
      ids.forEach((b) => {
        if (a >= b) return; // visit each unordered pair only once
        const diff = ledger[a][b] - ledger[b][a]; // Find net amount to be cleared between a and b
        if (diff > 0) { // If a owes b
          ledger[a][b] = diff;
          ledger[b][a] = 0;
        } else if (diff < 0) { // If b owes a
          ledger[b][a] = -diff;
          ledger[a][b] = 0;
        } else { // If they are settled, set both to 0
    ...
    /* ----------  shape the response ---------- */
    const balances : EnhancedBalance[] = memberDetails.map((m) => ({
      ...m,
    ...
    const userLookupMap : Record<Id<"users">,EnhancedGroupMember>= {};
    memberDetails.forEach((member) => {
        if (!member.id) return; // skip if no user found
        else userLookupMap[member.id] = member;
    });
