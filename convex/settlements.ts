...
export const createSettlement = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const caller : Doc<"users"> = await ctx.runQuery(internal.users.getCurrentUser);
    ...
    if (args.groupId) {
      ...
      const isMember = (uid : Id<"users">) => group.members.some((m) => m.userId === uid);
...
export const getSettlementData = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.entityType === "user") {
      ...
      /* ──────────────────────────────────────────────────────── group page */
      const group = await ctx.db.get(args.entityId as Id<"groups">);
      if (!group) throw new Error("Group not found");
      ...
      const list = Object.keys(balances).map((uid: string) => {
        const m = members.find((u) => u && u._id === uid);
        const { owed, owing } = balances[uid as Id<"users">];
        return {
