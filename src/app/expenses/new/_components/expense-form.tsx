...
import { RawSplit, SelectedGroup, ShortUser, Split } from "@/types";
...
interface ExpenseFormProps {
    type?: "individual" | "group"; // Default to individual
    onSuccess?: (id: string ) => void; // Callback when expense is created
}

export function ExpenseForm({ type = "individual", onSuccess } : ExpenseFormProps) {
  const [participants, setParticipants] = useState<ShortUser[]>([]);
  ...
  const createExpense = useDBMutation(api.expenses.createExpense);
  ...
  const onSubmit = async (data : NewExpenseData) => {
    try {
      const amount = parseFloat(data.amount);

      // Prepare splits in the format expected by the API
      const formattedSplits : Split[] = splits.map((split) => ({
        userId: split.userId,
        amount: split.amount,
          ...
          <Tabs
            ...
            onValueChange={(value : string) =>{
              if(value !== "equal" && value !== "percentage" && value !== "exact") {
                toast.error("Invalid split type selected");
                return;
              }
              // Update split type in form state
