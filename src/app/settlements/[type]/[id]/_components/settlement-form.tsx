...
import { BalanceDetail, GroupSettlement, SettlementData, UserSettlement } from "@/types";
...
export default function SettlementForm({ entityType, entityData, onSuccess }: SettlementFormProps) {
    ...
    const createSettlement = useDBMutation(api.settlements.createSettlement);
    ...
    try {
      ...
      const paidByUserId =
        data.paymentType === "youPaid"
          ? currentUser._id
          : (entityData as UserSettlement).counterpart.userId;
  ...
  const handleGroupSettlement = async (data : SettlementFormData, selectedUserId: Id<"users">) => {
    ...
    try {
      ...
      const selectedUser = (entityData as GroupSettlement).balances.find(
        (balance : BalanceDetail) => balance.userId === selectedUserId
      );
      ...
      await createSettlement.mutate({
        ...
        paidByUserId,
        receivedByUserId,
        groupId: (entityData as GroupSettlement).group.id,
      });
  ...
  const [selectedGroupMemberId, setSelectedGroupMemberId] = useState<Id<"users"> | undefined>(undefined);

  if (!currentUser) return null;

  // Render the form for individual settlement
  if (entityType === "user") {
    const otherUser = (entityData as UserSettlement).counterpart;
    const netBalance = (entityData as UserSettlement).netBalance;
  ...
  if (entityType === "group") {
    const groupMembers = (entityData as GroupSettlement).balances;
